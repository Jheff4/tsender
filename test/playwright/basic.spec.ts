...
test('should show error when user rejects transaction', async ({
  context,
  page,
  metamaskPage,
  extensionId
  ...
  // Click Send → MetaMask popup opens
  await page.getByRole('button', { name: 'Send' }).click()

  // USER REJECTS TX
  await metamask.rejectTransaction()

  // Assert UI handles rejection
  await expect(
    page.getByText('Transaction rejected')
  ).toBeVisible({ timeout: 15000 })

  // Ensure no success UI
  await expect(
    page.getByText('Success')
  ).not.toBeVisible()
})
